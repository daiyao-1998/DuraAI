...
class MCPToolKit:
    """有限元分析结果查询工具集，支持完整操作链（优化后支持多ID、多名称、多文件和多结果类型批量查询）"""
    ...
    def _stitch_screenshots(self, screenshot_paths: List[str], output_path: str = None, 
                           orientation: str = "vertical") -> str:
        """
        将多个截图拼接成一张图片，并在每张图片下方添加视图角度标注，完成后删除原始图片
        :param screenshot_paths: 截图文件路径列表
        :param output_path: 输出文件路径，默认为None，将在第一个截图同目录下生成
        :param orientation: 拼接方向，"vertical"（纵向）或"horizontal"（横向）
        ...
        if orientation == "vertical":
            ...
            stitched_image = Image.new('RGB', (total_width + len(images)*10, max_height + 50), color='white')
        ...
        for i, (img, path) in enumerate(zip(images, screenshot_paths)):
            ...
            if "case_" in base_name:
                label = f"工况 {base_name.split('case_')[1].split('_')[0]}"
            else:
                label = base_name.split('_')[-2].replace('_', ' ')
            
            # 粘贴图片
            if orientation == "vertical":
                stitched_image.paste(img, (0, offset))
                # 添加标注
                draw.text((10, offset + img.size[1] + 10), 
                         f"视图角度: {label}", 
                         fill="black", 
                         font=font)
                offset += img.size[1] + 50
            else:
                stitched_image.paste(img, (offset, 0))
                # 添加标注
                draw.text((offset + 10, img.size[1] + 10), 
                         label, 
                         fill="black", 
                         font=font)
        ...
        if output_path is None:
            ...
            orientation_suffix = "vertical" if orientation == "vertical" else "horizontal"
            output_path = os.path.join(base_dir, f"stitched_image_{orientation_suffix}_{timestamp}.png")
    ...
    def _get_multi_entity_results(
        ...
        """
        通用多实体结果查询底层方法（支持不同工况对应不同实体ID或名称，以及多文件多结果类型）
        :param file_category_map: 字典，键为文件标识，值为包含"result_file"和"result_category"的字典
        ...
        """
        # 参数验证
        if not file_category_map:
            return "file_category_map参数不能为空"
        # 必须提供ID或名称中的一种
        ...
        # 为每个文件和结果类型构建命令
        for file_id, file_info in file_category_map.items():
            result_file = file_info.get("result_file")
            result_category = file_info.get("result_category")
            
            if not result_file or not result_category:
                return f"file_category_map中的条目必须包含'result_file'和'result_category'键: {file_id}"
            
            # 添加文件标识信息到日志
            all_commands.append(f'options message "===== 开始处理文件: {result_file}，结果类型: {result_category} ====="')
            ...
            for case_id, entities in case_entity_map.items():
                ...
                commands.append('window clearcreate 3d keepses')    
            
            all_commands.extend(commands)
        
        # 执行命令并返回日志
        result = self._run_commands(all_commands, query)
        return f"多{entity_type}多文件结果查询日志:\n{result}"
    ...
    def get_multi_node_results(
        ...
        """
        获取单个或者多个节点在指定工况下的结果（支持不同工况对应不同节点ID或名称，以及多文件多结果类型）
        :param file_category_map: 字典，键为文件标识，值为包含"result_file"和"result_category"的字典
    ...
    def get_multi_element_results(
        ...
        """
        获取单个或者多个单元在指定工况下的结果（支持不同工况对应不同单元ID或名称，以及多文件多结果类型）
        :param file_category_map: 字典，键为文件标识，值为包含"result_file"和"result_category"的字典
    ...
    def get_multi_part_results(
        ...
        支持通过ID或名称查询，如STU-0000116263_C611，以及多文件多结果类型
        :param file_category_map: 字典，键为文件标识，值为包含"result_file"和"result_category"的字典
    ...
    def get_multi_material_results(
        ...
        """
        获取单种或者多种材料在指定工况下的结果
        支持通过ID或名称查询，如HC340LAD+Z，以及多文件多结果类型
        :param file_category_map: 字典，键为文件标识，值为包含"result_file"和"result_category"的字典
    ...
    def get_multi_set_results(
        ...
        支持通过ID或名称查询，如LAC-51010131，以及多文件多结果类型
        :param file_category_map: 字典，键为文件标识，值为包含"result_file"和"result_category"的字典
    ...
    def get_max_result_for_entities(
        ...
        支持多文件多结果类型查询，当ids_per_case和names_per_case都为空时，默认查询全部模型结果
        :param file_category_map: 字典，键为文件标识，值为包含"result_file"和"result_category"的字典
        ...
        for file_id, file_info in file_category_map.items():
            ...
            for case_id, entities in case_entity_map.items():
                ...
                if node_or_element_result == "node":
                    ...
                    return "不支持的结果类型"
                commands.extend(['window clearcreate 3d keepses'])
            all_commands.extend(commands)
        
        # 执行命令并返回日志
        result = self._run_commands(all_commands, query)
        return f"{entity_type}最大结果多文件查询日志:\n{result}"
    ...
    def get_model_info(
        ...
        支持同时查询多种实体类型，通过ids_per_case或names_per_case指定不同工况下的查询对象
        当两个参数都为空时，查询所有信息，同时支持多文件多类型查询
        :param file_info_map: 字典，键为文件标识，值为包含"result_file"、"info_types"（列表）和"result_category"的字典
        :param ids_per_case: 字典，键为工况ID，值为该工况下的实体ID列表
        :param names_per_case: 字典，键为工况ID，值为该工况下的实体名称列表
        :param query: 查询需求，用于从日志中提取相关信息
        :return: 查询结果日志或提取的相关信息
        ...
        try:
            ...
            for file_id, file_info in file_info_map.items():
                result_file = file_info.get("result_file")
                info_types = file_info.get("info_types", [])  # 改为列表，支持多种类型
                result_category = file_info.get("result_category", "Mises")  # 默认使用Mises
                
                if not result_file or not info_types:
                    return f"file_info_map中的条目必须包含'result_file'和'info_types'键: {file_id}"
                ...
                # 添加文件标识信息到日志
                all_commands.append(f'options message "===== 开始处理文件: {result_file}，信息类型: {",".join(info_types)} ====="')
                ...
                for case_id, entities in case_entity_map.items():
                    if case_id != 0:
                        commands.append(f'options state "{case_id}"')
                    commands.append(f'options message "----- 开始查询工况 {case_id} 的模型信息 -----"')
                    ...
                    for info_type in info_types:
                        ...
                        commands.append(f'options message "--- 查询{info_type}信息 ---"')
            ...
            return f"模型信息多文件查询日志文件内容:\n{self._run_commands(all_commands, query)}"
        except (FileNotFoundError, ValueError) as e:
            return str(e)
    ...
    def capture_screenshots(
        ...
        for case_id, entities in case_entity_map.items():
            ...
            if use_all:
                # 显示整个模型
                commands.append(f'options message "----- 开始查询工况 {case_id} 的整个模型最大结果 -----"')
            ...
            for view in views:
                ...
                if use_all:
                    # 整个模型的截图
                    filename = f"{os.path.basename(result_file).split('.')[0]}_{result_category}_entire_model_case_{case_id}_{view.replace(' ', '_')}_{timestamp}.png"
                elif use_ids:
                    filename = f"{os.path.basename(result_file).split('.')[0]}_{result_category}_{entity_type}_case_{case_id}_{view.replace(' ', '_')}_{timestamp}.png"
                else:
                    filename = f"{os.path.basename(result_file).split('.')[0]}_{result_category}_{entity_type}_case_{case_id}_{view.replace(' ', '_')}_{timestamp}_by_name.png"
            ...
            # 纵向拼接同一工况的不同视角截图
            case_screenshot = self._stitch_screenshots(screenshot_paths, orientation="vertical")
            # 提取拼接后的图片路径
            case_img_path = case_screenshot.split("src='/images/")[1].split("'")[0]
            case_full_path = os.path.join(output_dir, case_img_path)
            case_screenshot_paths.append(case_full_path)
        
        # 横向拼接不同工况的截图
        if len(case_screenshot_paths) > 1:
            final_html = self._stitch_screenshots(case_screenshot_paths, orientation="horizontal")
        else:
            # 如果只有一个工况，直接使用该工况的截图
            final_html = f"<img src='/images/{os.path.basename(case_screenshot_paths[0])}' style='max-width: 100%; height: auto;'>"
        
        return f"<div>{final_html}</div>"
